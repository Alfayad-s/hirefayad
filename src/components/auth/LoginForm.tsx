...
export function LoginForm({ embedded, onSuccess, onSwitchToSignup }: LoginFormProps = {}) {
  ...
  async function onSubmit(data: LoginInput) {
    ...
    if (session?.user?.name) {
      try {
        localStorage.setItem("serviceFunnel_userName", session.user.name);
      } catch {
    ...
    >
      <Card className="w-full max-w-md border-2 shadow-lg shadow-primary/5">
        <CardHeader className="space-y-1 text-center">
          ...
          <CardDescription>
            Sign in to your account to apply coupons and get quotes.
          </CardDescription>
        </CardHeader>
        <form onSubmit={handleSubmit(onSubmit)}>
